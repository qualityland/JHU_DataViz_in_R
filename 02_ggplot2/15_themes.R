...
# import survey data
cces <- read_csv("02_ggplot2/data/cces.csv")
# cces <- 
#   read_csv(
#     url(
...
plot_data <-
  select(cces, "CC18_308a", "ideo5", "educ", "faminc_new", "employ") %>%
  drop_na()
...
ggplot(plot_data, aes(y = CC18_308a, x = ideo5, color = educ, size = faminc_new)) +
  geom_jitter() +
  # use scale_color_gradient to show continuous change in a numeric variable
  scale_color_gradient(low = "gray", high = "purple")


# use employment as categorical variable
plot_data$employ_cat <-
  recode(
    plot_data$employ,
    `1` = "Full-time",
    `2` = "Part-time",
...
# instead of scale color gradient, use scale color brewer for
# the discrete variable you have created
ggplot(plot_data, aes(y = CC18_308a, x = ideo5, color = employ_cat)) +
  geom_jitter() +
  scale_color_brewer(palette = "RdYlGn") # red-yellow-green

# there are many palettes
# see https://www.r-graph-gallery.com/38-rcolorbrewers-palettes
