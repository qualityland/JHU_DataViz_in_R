...
# make a scatter plot of lifetime high scores in tetris against cumulative hours
# playing tetris likely to see positive correlation
...
cel %>%
  ...
  geom_point() +
  geom_text_repel(
    filter(cel, congress == 115 & all_pass > 8),
    mapping = aes(x = dwnom1, y = all_pass, label = thomas_name)
  ) +
  annotate(
    "rect",
