...
japan_big_cities <-
  filter(my_cities, country.etc == "Japan" & pop > 500000)
...
japan_big_cities$qual <-
  sample(LETTERS[1:5], nrow(japan_big_cities), replace = TRUE)

ggplot(data = japan,
       mapping = aes(x = long, y = lat, group = group)) +
  geom_polygon(color = "black", fill = "white") +
  geom_point(data = japan_big_cities, aes(
    x = long,
    y = lat,
    group = NULL,
    color = qual,
