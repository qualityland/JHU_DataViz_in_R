...
boxplot(dat$faminc_new, 
        main = "Stefan's Boxplot of Family Income", 
        xlab = "Family Income")
```
...
CC18_325a_support = recode(dat$CC18_325a, `1` = 1, `2` = 0)
CC18_325d_support = recode(dat$CC18_325d, `1` = 1, `2` = 0)

dat_new <-  dat %>% 
  select(CC18_325a, CC18_325d) %>% 
  mutate(tax_scale = CC18_325a_support + CC18_325d_support)
...
dat %>% 
  group_by(region) %>% 
  summarize(Trump_Approve_Mean = mean(CC18_308a))
```
...
dat %>% 
  filter(investor == 2,
         faminc_new %in% 5:10) %>% 
  summarize("Mean Religion Imp." = mean(pew_religimp),
...
```{r problem11}
kable(dat %>% summarize("Mean" = mean(faminc_new),
